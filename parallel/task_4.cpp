...
int main(int argc, char ** argv) {//mpirun -np <P> ./prog <P_row> <P_col> <матрица_а> <вектор_b> <вектор_c>
    ...
    int32_t m, n;
/*    if (!fread(&m, sizeof(int32_t), 1, f_1)) {
    ...
    fread(&m, sizeof(int32_t), 1, f_1);
    fread(&n, sizeof(int32_t), 1, f_1);
    fread(&n, sizeof(int32_t), 1, f_2);

    int32_t *B = (int32_t *)calloc(n * 1, sizeof(*B));
    int32_t *C = (int32_t *)calloc(m * 1, sizeof(*C));

/*    if (!fread(B, sizeof(int32_t), n * 1, f_2)) {
        cout << "Error: fread() failed" << endl;
        return 1;
    }*/
    fread(B, sizeof(int32_t), n * 1, f_2);
    ...
    int32_t *A = (int32_t *)calloc(new_n * new_m, sizeof(*A));

    fseek(f_1, offset * sizeof(int32_t), 1);

    for (int k = 0; k < new_m; k++) {
/*        if (!fread(A + k * new_n, sizeof(int32_t), new_n, f_1)) {
            cout << "Error: fread() failed" << endl;
            return 1;
        }*/
        fread(A + k * new_n, sizeof(int32_t), new_n, f_1);
        fseek(f_1, (n - new_n)*sizeof(int32_t), 1);
    }
    /*    for (int k = 0; k < new_m; k++) {
...
//суммирование и вывод ответа в файл
    if (rank == 0) {
        ...
        int32_t *buf = (int32_t *)calloc(m * 1, sizeof(*buf));
        MPI_Status status;
